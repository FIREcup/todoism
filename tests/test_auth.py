...
class AuthTestCase(unittest.TestCase):
    ...
    def test_login(self):
        response = self.client.post(url_for('auth.login'), json=dict(username='yang', password='123'))
        data = response.get_json()
        self.assertEqual(data['message'], 'Login success.')

    def test_logout(self):
        self.client.post(url_for('auth.login'), json=dict(username='yang', password='123'), follow_redirects=True)
    ...
    def test_get_test_account(self):
        ...
        self.assertINotNone(User.query.filter_by(username=data['username']))
