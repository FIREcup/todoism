...
def get_item_body():
    ...
    if body is None or str(body).strip() == '':
        raise ValidationError('The item body way empty or invalid.')
    return body
...
class ActiveItemsAPI(MethodView):
    ...
    def get(self):
        ...
        pagination = Item.query.with_parent(g.current_user).filter_by(done=False).paginate(
            page=page, per_page=current_app.config['TODOISM_ITEM_PER_PAGE']
        )
        items = pagination.items
        current = url_for('.items', page=page, _external=True)
        prev = None
...
api_v1.add_url_rule('/user/items/<int:item_id>', view_func=ItemsAPI.as_view('item'),
                    methods=['GET', 'PUT', 'PATCH', 'DELETE'])
api_v1.add_url_rule('/user/items/active', view_func=ItemsAPI.as_view('active_items'), methods=['GET'])
api_v1.add_url_rule('/user/items/completed', view_func=ItemsAPI.as_view('completed_items'), methods=['GET', 'DELETE'])
